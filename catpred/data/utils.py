...
from catpred.rdkit_module import make_mol
...
def get_constraints(
    path: str, target_columns: List[str], save_raw_data: bool = False
    ...
    if save_raw_data:
        ...
        raw_constraints_data = np.transpose(
            raw_constraints_data
        )  # each is num_data x num_columns
    else:
...
def get_data(
    ...
    if args is not None:
        ...
        bond_descriptors_path = (
            bond_descriptors_path
            if bond_descriptors_path is not None
            else args.bond_descriptors_path
        )
        constraints_path = (
            constraints_path if constraints_path is not None else args.constraints_path
        )
        max_data_size = (
    ...
    with open(path) as f:
        ...
        (
            ...
            all_features,
            all_phase_features,
            all_constraints_data,
            all_raw_constraints_data,
            all_weights,
            all_gt,
            all_lt,
        ) = ([], [], [], [], [], [], [], [], [], [], [], [], [])
        ...
        for i, row in enumerate(tqdm(reader)):
            ...
            if not protein_records is None:
                sequence_features, _ = sequence_feat_getter(
                    protein_record["seq"], name=pdbname, device="cpu"
                )
            ...
            for column in target_columns:
                ...
                if value in ["", "nan"]:
                    ...
                    target = np.array(json.loads(value))
                    if (
                        len(target.shape) == 1 and column in args.atom_targets
                    ):  # Atom targets saved as 1D list
                        atom_targets.append(target)
                        targets.append(target)
                    elif (
                        len(target.shape) == 1 and column in args.bond_targets
                    ):  # Bond targets saved as 1D list
                        ...
                        raise ValueError(
                            f"Unrecognized targets of column {column} in {path}."
                        )
                else:
        ...
        if args is not None and args.bond_descriptors is not None:
            try:
                descriptors = load_valid_atom_or_bond_features(
                    bond_descriptors_path, [x[0] for x in all_smiles]
                )
            except Exception as e:
                raise ValueError(
        ...
        data = MoleculeDataset(
            [
                MoleculeDatapoint(
                    ...
                    phase_features=(
                        all_phase_features[i] if phase_features is not None else None
                    ),
                    atom_features=(
                        atom_features[i] if atom_features is not None else None
                    ),
                    atom_descriptors=(
                        atom_descriptors[i] if atom_descriptors is not None else None
                    ),
                    bond_features=(
                        bond_features[i] if bond_features is not None else None
                    ),
                    bond_descriptors=(
                        bond_descriptors[i] if bond_descriptors is not None else None
                    ),
                    constraints=(
                        all_constraints_data[i]
                        if constraints_data is not None
                        else None
                    ),
                    raw_constraints=(
                        all_raw_constraints_data[i]
                        if raw_constraints_data is not None
                        else None
                    ),
                    ...
                    overwrite_default_bond_features=(
                        args.overwrite_default_bond_features
                        if args is not None
                        else False
                    ),
                )
