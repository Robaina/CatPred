...
def setup_model_weights(weights_dir=None):
    ...
    try:
        # Create all parent directories with appropriate permissions
        cache_dir.parent.mkdir(parents=True, exist_ok=True)
        cache_dir.mkdir(exist_ok=True)
        
        # Ensure directory has write permissions
        os.system(f"chmod -R 777 {cache_dir.parent}")
        ...
        for filename in weight_files:
            ...
            if not src_file.exists():
                print(f"Warning: Weight file {filename} not found in {weights_dir}")
                success = False
                continue
        ...
        print(f"Error setting up cache directory: {str(e)}")
        return False
